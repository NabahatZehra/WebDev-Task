...
exports.postUpdateOrderStatus = async (req, res) => {
  try {
    ...
    let allowed = false;
    if (order.status === 'Placed' && newStatus === 'Processing') allowed = true;
    if (order.status === 'Processing' && newStatus === 'Delivered') allowed = true;

    if (!allowed) {
      req.session.errorMessage = `Invalid transition from ${order.status} to ${newStatus}.`;
      return res.redirect('/admin/orders');
    }
    ...
    req.session.successMessage = `Order #${order._id} updated to ${newStatus}.`;
    res.redirect('/admin/orders');

  } catch (error) {
    console.error('Update order status error:', error);
    req.session.errorMessage = 'Error updating order status.';
    res.redirect('/admin/orders');
  }
};
